...
// High-res curated Iraqi cityscape & thematic photography
...
export async function getLiveIraqNews(options: LiveNewsOptions = {}): Promise<CityNews[]> {
    ...
    try {
      ...
      for (const item of govArticles) {
        if (!seenTitles.has(item.title) && !seenIds.has(item.id)) {
          seenTitles.add(item.title);
          seenIds.add(item.id);
          // ensure governorateId is tagged
  ...
  // 2. Fetch General Iraq National News Feed
  try {
  ...
  // 3. Fetch INA (وكالة الأنباء العراقية الرسمية واع)
  try {
  ...
  // 4. If we got items, update cache
  ...
  // 5. Ultimate Fallback: High Quality Real Time Headlines with dynamic Arabic date stamps
