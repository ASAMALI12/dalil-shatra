...
export const AccountView: React.FC<AccountViewProps> = ({
  ...
  const emergencyNumbers = [
    { title: 'شرطة النجدة العراقية', number: '104', icon: '🚓' },
    { title: 'الدفاع المدني والإطفاء', number: '115', icon: '🚒' },
    { title: 'الإسعاف الفوري المركزي', number: '122', icon: '🚑' },
    { title: 'جهاز الأمن الوطني', number: '131', icon: '🛡️' },
    { title: 'طوارئ الكهرباء والصيانة', number: '159', icon: '⚡' },
    { title: 'شكاوى واستعلامات الخدمات', number: '5666', icon: '🏢' },
  ];
    ...
    <div className="space-y-5 pb-20">
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-xs flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-tr from-slate-900 via-slate-800 to-emerald-700 text-white font-display text-2xl font-bold shadow-md ring-4 ring-slate-100">
            {isManagerUnlocked ? 'ع' : '👤'}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-display text-lg sm:text-xl font-bold text-slate-900">
                {isManagerUnlocked ? 'مدير تطبيق دليل العراق' : userName}
              </h2>
              {isManagerUnlocked ? (
                <span className="rounded-md bg-emerald-100 border border-emerald-200 px-2 py-0.5 text-[10px] font-bold text-emerald-800 flex items-center gap-1">
            ...
            <p className="text-xs text-slate-500 font-semibold mt-0.5">
              {isManagerUnlocked ? 'إدارة النظام المركزية والمحفظة' : 'دليل العراق الموحد لجميع المحافظات والأقضية'}
            </p>
          </div>
        </div>

        {/* Manager Mode Quick Status - Only shown if manager is authenticated */}
        {isManagerUnlocked && (
          <button
            type="button"
            onClick={lockManager}
        ...
        {myClaimedStores.length > 0 ? (
          ...
          <div className="rounded-2xl bg-amber-50/70 border border-amber-200/80 p-4 text-center space-y-2.5">
            <p className="text-xs text-amber-900 leading-relaxed font-semibold">
              هل أنت صاحب محل أو عيادة أو نشاط تجاري مدرج بالفعل في دليل العراق؟
            </p>
            <p className="text-[11px] text-amber-800 leading-relaxed">
              جميع المتاجر في التطبيق حقيقية، ويحق لك المطالبة بمتجرك فوراً بمجرد تأكيد رقم هاتفك المسجل عبر رمز التحقق (OTP) السريع.
            </p>
            <button
              type="button"
      ...
      {/* 2. STORE REGISTRATION & PHONE VERIFICATION */}
        ...
        <div className="flex items-center gap-3 text-center sm:text-right">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-800 mt-0.5">
              يدخل المستخدم بدون تسجيل، وعند فتح متجر يتم التحقق الصارم من رقم الهاتف العراقي وبث إشعار لجميع الهواتف فوراً.
            </p>
          </div>
        </div>
      ...
      {/* 3. SECURE MANAGER ACCESS CARD (Shown only if manager is unlocked, or hidden for normal users) */}
      {isManagerUnlocked && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-right">
                ...
                <p className="text-[11px] text-slate-400">تنبيهات العروض والمحلات الجديدة في منطقتك</p>
              </div>
            </div>
      ...
      {/* Emergency Phone Numbers in Iraq */}
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <PhoneCall className="h-4 w-4 text-red-600" />
          <h3 className="font-display text-base font-bold text-slate-900">
            أرقام الطوارئ والخدمات المباشرة في العراق
          </h3>
        </div>
      ...
      <div className="text-center text-xs text-slate-400 pt-4 space-y-1">
        <p className="font-bold text-slate-600">تطبيق دليل العراق الموحد</p>
        <p>الإصدار 3.0.0 • يغطي جميع المحافظات والأقضية العراقية</p>
      </div>
    </div>
  );
};
