...
export const OpenStoreModal: React.FC<OpenStoreModalProps> = ({
  ...
  const initialGovId = currentLocation.governorateId || 'baghdad';
  const [governorateId, setGovernorateId] = useState(initialGovId);
  const [districtId, setDistrictId] = useState(() => {
    if (currentLocation.districtId && currentLocation.districtId !== 'all') {
      return currentLocation.districtId;
    }
    const currentGov = IRAQ_GOVERNORATES.find((g) => g.id === initialGovId);
    return currentGov?.districts[0]?.id || 'rusafa';
  });
  ...
  useEffect(() => {
    if (!availableDistricts.some((d) => d.id === districtId)) {
      setDistrictId(availableDistricts[0]?.id || 'center');
    }
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    ...
    const phoneCheck = validateIraqPhone(phone);
    const cleanPhone = phoneCheck.formattedDisplay || phoneCheck.normalized;
    const cleanWhatsapp = phoneCheck.normalized;
        ...
        <div className="overflow-y-auto p-5 space-y-4 flex-1">
          ...
          {step === 'info' && (
            <form onSubmit={handleProceedToVerification} className="space-y-4">
              ...
              {/* 3. Location: Governorate & District */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block font-display text-xs font-bold text-slate-700">
                    المحافظة *
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      placeholder="الشارع العام، قرب السوق، مجاور..."
            ...
            <div className="py-4 text-center space-y-4 animate-in zoom-in-95 duration-200">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 max-w-xs mx-auto mt-1">
                  أصبح متجر <strong>"{createdStore.name}"</strong> متاحاً الآن في دليل العراق الذكي لجميع الزبائن، وتم بث إشعار تلقائي لجميع الهواتف!
                </p>
              </div>
