...
export const EditStoreModal: React.FC<EditStoreModalProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Keep existing images list, updating the first item if cover changed
    let updatedImages = Array.isArray(store.images) ? [...store.images] : [];
    if (finalCover) {
      if (!updatedImages.includes(finalCover)) {
        updatedImages = [finalCover, ...updatedImages];
      } else {
        // move finalCover to front
        updatedImages = [finalCover, ...updatedImages.filter((img) => img !== finalCover)];
      }
    }
    ...
    const updatedData: Partial<DirectoryItem> = {
      ...
      imageUrl: finalCover,
      images: updatedImages.length > 0 ? updatedImages : [finalCover],
      isOpen: isOpenNow,
    };
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5 space-y-2.5">
              <label className="block font-display text-xs font-bold text-slate-800 flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>صورة الغلاف الرئيسية للمتجر ⭐</span>
                </span>
                <span className="text-[10px] text-slate-400 font-normal">
                  تظهر في الصفحة الرئيسية وقوائم الدليل
                </span>
              </label>

              <div className="flex items-center gap-3">
                {/* Preview */}
                <div className="relative h-20 w-24 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 shrink-0 shadow-2xs">
                  <img
                  ...
                  />
                  {isUploadingCover && (
                    <div className="absolute inset-0 bg-slate-900/60 flex items-center justify-center text-[9px] text-white font-bold">
                      معالجة...
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  ...
                  <label
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-bold text-xs px-3 py-1.5 shadow-2xs cursor-pointer transition-all"
                  >
